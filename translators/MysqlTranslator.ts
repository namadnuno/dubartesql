...
export class MysqlTranslator<T> {
    ...
    sqlMidle() {
        if (this.queryBuilder.type == 'insert') {
            return `(${Object.keys(this.queryBuilder.fieldsToUpdateOrInsert).join(',')}) values ('${Object.values(this.queryBuilder.fieldsToUpdateOrInsert).join("','")}')`;
        }

        if (this.queryBuilder.wheres.length > 0) {
            return "where " + this.queryBuilder.wheres.reduce((acc: any, where:any) => {
    ...
    sqlSet(){
        if (this.queryBuilder.type == 'update') {
            return "SET " + Object.keys(this.queryBuilder.fieldsToUpdateOrInsert).reduce((acc: any, key) => {
                return [ ...acc, `${key}="${this.queryBuilder.fieldsToUpdateOrInsert[key]}"`]
            }, []).join(",");
        }
